  ...
  return (
    <main className="pb-28 lg:pb-12">
      <div className="max-w-3xl mx-auto px-4 md:px-6 lg:px-8 pt-5 md:pt-7 space-y-5 md:space-y-6">
        <PageHeader
          title="설정"
          description="어떤 AI를 쓸지 고르고 본인의 API 키를 등록해 주세요. 키는 이 기기의 브라우저에만 저장되며, 저희 서버에는 저장되지 않아요."
          accent="neutral"
        />

        <Card>
          <StepHeader step={1} title="AI 프로바이더" accent="coral" />
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            {(Object.keys(PROVIDER_LABELS) as ProviderId[]).map((p) => {
              ...
              return (
                ...
                >
                  <div className="font-extrabold text-base tracking-[-0.02em]">
                    {PROVIDER_LABELS[p]}
                  </div>
                </button>
              );
        ...
        <Card>
          <StepHeader step={2} title="모델" accent="coral" />
              ...
              return (
                <label
                  ...
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-lg border cursor-pointer transition-all duration-150 ${
                    active
                      ? "border-coral bg-coral-bg/50"
                      : "border-line-light hover:border-line-medium"
                  }`}
                >
                  <input
                    type="radio"
          ...
          <details className="mt-3 text-sm">
            ...
            <input
              ...
              className="mt-2 w-full px-3 py-2 text-sm rounded-lg border border-line-medium focus:border-coral focus:outline-none focus:ring-3 focus:ring-coral-bg font-mono"
            />
        ...
        <Card>
          <StepHeader step={3} title="API 키" accent="coral" />
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-3 py-2 rounded-lg border border-line-medium focus:border-coral focus:outline-none focus:ring-3 focus:ring-coral-bg font-mono text-sm"
              autoComplete="off"
            />
            <button
              onClick={() => setShowKey((v) => !v)}
              className="px-3 py-2 text-sm border border-line-medium rounded-lg hover:bg-warm font-bold"
            >
              {showKey ? "숨기기" : "보기"}
            </button>
          </div>
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="mt-3 inline-block text-sm text-coral hover:underline font-bold"
          >
        ...
        <Card className="flex flex-wrap gap-3 items-center">
          <button
            ...
            className="px-6 py-3 bg-coral text-white rounded-xl font-extrabold text-base hover:bg-coral-light hover:-translate-y-px disabled:bg-line-medium disabled:cursor-not-allowed transition-all duration-150 shadow-coral"
          >
            저장
          </button>
          <button
            onClick={clear}
            className="px-4 py-3 text-sm text-ink-secondary hover:text-coral font-medium"
          >
            모두 초기화
          </button>
          {savedAt && (
            <span className="text-sm text-sage font-bold">
              ✓ 저장됨 ({savedAt})
            </span>
          )}
        </Card>
        ...
        <Card className="bg-warm/40">
          <h3 className="font-bold text-base text-ink mb-2 tracking-[-0.02em]">
            개인정보 보호 안내
          </h3>
          <ul className="space-y-1.5 list-disc pl-5 text-sm text-ink-secondary leading-relaxed">
            <li>
              API 키는 <strong className="text-ink">이 기기의 브라우저</strong>
              에만 저장됩니다 (
            ...
            <li>
              ...
              기기 변경 시에는 데이터 관리의 내보내기/가져오기 기능을 사용해
              주세요.
            </li>
            <li>공용 PC에서는 작업 후 반드시 "모두 초기화"로 키를 지워주세요.</li>
          </ul>
        </Card>
      </div>
    </main>

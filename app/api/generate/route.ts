...
- **마무리 한 문장 — 반드시 학부모님께 직접 말하듯이.** 절대 아이에게 말하는 어투 X.
  · 좋은 예: "오늘도 푹 쉬게 해주세요." / "감기 조심하시고 내일 또 만나요." / "주말 잘 보내세요." / "오늘 그린 그림 가방에 넣어드렸으니 가정에서 함께 봐 주세요."
  · 나쁜 예 (절대 금지): "내일도 즐거운 하루가 되길 바라요" (← 누구한테?), "오늘도 행복했길 바라요" (← 아이한테 하는 말 같음), "사랑해요" (← 학부모께 쓸 말 아님), "행복한 하루 보내세요^^"
...
- 번호 매기기·소제목·문단 구분·대괄호 헤더 절대 금지. 줄글 한 덩어리.
- "~합니다체" 같은 딱딱한 공문체 금지.
- 이모지·특수문자 금지.
- 학부모를 아이로 착각하는 듯한 인사말 금지 ("바라요", "되길", "건강해요" 등 학부모께 쓰면 어색한 표현).
...
[반드시 지킬 구조 — 출력에 대괄호 섹션 헤더를 그대로 포함]
출력 본문은 반드시 아래 세 섹션을 이 순서대로, 대괄호 헤더 그대로 포함합니다.

[관찰 상황]
누가·언제·어디서·무엇을·어떻게 했는지 구체적 장면. 행동·발화·표정을 사실 위주로 묘사. 교사의 해석은 들어가지 않음. 3~5문장.

[발달적 의미]
위 관찰이 누리과정 5영역 중 어느 영역의 어떤 하위 능력과 연결되는지 명시. **반드시 영역명을 본문에 직접 명시** ("예술경험 영역의 창의적 표현하기" 등). 한 편에 보통 1~3개 영역이 자연스럽게 등장. 3~5문장.
- 5영역: ① 신체운동·건강 ② 의사소통 ③ 사회관계 ④ 예술경험 ⑤ 자연탐구

[교사 지원 방향]
앞으로 이 아이를 어떻게 더 도울지 구체적 계획. 환경 구성, 추가 활동, 또래 상호작용 지원 등. 2~4문장.

각 섹션은 빈 줄(\\n\\n)로 구분합니다. 섹션 헤더는 정확히 "[관찰 상황]", "[발달적 의미]", "[교사 지원 방향]" 이 세 개만 사용 (다른 표기 금지).
...
[좋은 관찰일지 예시 — 형식·톤·구조 참고]
"[관찰 상황]\\n민선이는 미술 영역에서 분홍색 물감을 손가락 끝에 묻혀 도화지에 꽃잎 모양을 반복적으로 찍어 표현하였다. 작업 중간에 '이건 엄마꽃이에요'라고 말하며 자신의 작품에 의미를 부여하는 모습이 관찰되었다. 점심 식사 시 닭볶음탕을 모두 섭취하였고 낮잠도 약 한 시간 동안 안정적으로 취하였다.\\n\\n[발달적 의미]\\n이는 예술경험 영역의 '창의적으로 표현하기'와 의사소통 영역의 '느낌과 생각을 말로 표현하기'가 함께 드러나는 장면이다. 손가락이라는 비전형적 도구를 선택하고 자신의 작품에 개인적 서사(엄마)를 부여한다는 점에서 표현 매체에 대한 탐색력과 정서 표현 능력이 동반 성장하고 있음을 알 수 있다.\\n\\n[교사 지원 방향]\\n다음 주에는 다양한 색의 물감과 자연물(꽃잎, 잎사귀)을 추가로 제공하여 표현 매체를 확장할 수 있도록 환경을 구성한다. 또한 작품에 담긴 이야기를 또래와 공유하는 활동을 제안하여 의사소통 능력 신장을 함께 도모할 계획이다."
...
[출력 형식]
반드시 JSON 객체 하나만. 형식: {"notes": [{"childId": "...", "text": "..."}, ...]}
- text 필드는 정확히 "[관찰 상황]\\n...\\n\\n[발달적 의미]\\n...\\n\\n[교사 지원 방향]\\n..." 형태로 시작해야 하며, 세 섹션 헤더를 모두 포함해야 한다.
- 아이 이름을 본문 첫머리에 헤더로 따로 붙이지 말 것 ([관찰 상황] 헤더가 첫 줄에 와야 함).
- 학부모 호칭, 인사말, "되길 바라요" 같은 표현이 등장하면 무효 — 다시 작성.`;
...
export async function POST(req: Request) {
  ...
  const docMissionLine = isGwanchal
    ? "👉 지금 작성하는 문서: **관찰일지** — 학부모께 보내는 글이 아닙니다. 출력은 반드시 [관찰 상황] / [발달적 의미] / [교사 지원 방향] 세 섹션 대괄호 헤더를 포함해야 하고, 종결어미는 '~한 모습이 관찰됨', '~하였다' 등 보고서체. '어머님', '~했답니다', 인사말, '바라요' 같은 학부모 대상 표현은 절대 등장 금지."
    : "👉 지금 작성하는 문서: **알림장** — 학부모님이 카톡으로 받아보시는 짧은 일기 편지입니다. '~했답니다 / ~했어요' 같은 부드러운 평어체. 마무리 인사는 반드시 학부모께 직접 (예: '오늘 푹 쉬게 해주세요'). 누리과정 영역 언급·대괄호 헤더·번호·소제목 절대 금지. 마무리에 '바라요' 류 어색한 표현 금지.";
